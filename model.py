...
class GRUnet(nn.Module):
    def __init__(self,  embedding_dim, hidden_dim, layer_dim=2):
        """
        ...
        # GRU + 全连接
        self.gru = nn.GRU(900, hidden_dim, layer_dim,
                          batch_first=True)
...
class DialogueGCNModel(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future,
        ...
        if self.graph_type=='relation':
            if not self.multi_modal:
                ...
                if 'a' in self.modals:
                    self.graph_net_a = GraphNetwork(2*D_e, n_classes, n_relations, max_seq_len, graph_hidden_size, dropout, self.no_cuda, self.use_GCN, self.return_feature)
                if 'v' in self.modals:
                    self.graph_net_v = GraphNetwork(2*D_e, n_classes, n_relations, max_seq_len, graph_hidden_size, dropout, self.no_cuda, self.use_GCN, self.return_feature)
                if 'l' in self.modals:
                    self.graph_net_l = GraphNetwork(2*D_e, n_classes, n_relations, max_seq_len, graph_hidden_size, dropout, self.no_cuda, self.use_GCN, self.return_feature)
            ...
            if not self.multi_modal:
                ...
                self.graph_net = GCNII(nfeat=2*D_e, nlayers=Deep_GCN_nlayers, nhidden=graph_hidden_size, nclass=n_classes, dropout=self.dropout, lamda = 0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue)  #最后一个参数是调是否利用
            else:
                if 'a' in self.modals:
                    self.graph_net_a = GCNII(nfeat=2*D_e, nlayers=Deep_GCN_nlayers, nhidden=graph_hidden_size, nclass=n_classes, dropout=self.dropout, lamda = 0.5, alpha=0.1, variant=True, return_feature = self.return_feature, use_residue=self.use_residue)
                if 'v' in self.modals:
                    self.graph_net_v = GCNII(nfeat=2*D_e, nlayers=Deep_GCN_nlayers, nhidden=graph_hidden_size, nclass=n_classes, dropout=self.dropout, lamda = 0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue)
                if 'l' in self.modals:
                    self.graph_net_l = GCNII(nfeat=2*D_e, nlayers=Deep_GCN_nlayers, nhidden=graph_hidden_size, nclass=n_classes, dropout=self.dropout, lamda = 0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue)
            print("construct "+self.graph_type,"with",Deep_GCN_nlayers,"layers")
        elif self.graph_type=='MMGCN' or self.graph_type=='MMGCN2':
            if self.graph_type == 'MMGCN':
                self.graph_model = MM_GCN(a_dim=2*D_e, v_dim=2*D_e, l_dim=2*D_e, n_dim=2*D_e, nlayers=64, nhidden=graph_hidden_size, nclass=n_classes, dropout=self.dropout, lamda=0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue, n_speakers=n_speakers, modals=self.modals, use_speaker=self.use_speaker, use_modal=self.use_modal)
            else:
                self.graph_model = MM_GCN2(a_dim=2 * D_e, v_dim=2 * D_e, l_dim=2 * D_e, n_dim=2 * D_e, nlayers=64,
                                           nhidden=graph_hidden_size, nclass=n_classes, dropout=self.dropout, lamda=0.5,
                                           alpha=0.1, variant=True, return_feature=self.return_feature,
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None):
        if self.base_model == "DialogRNN":
        ...
        if not self.multi_modal:
            ...
            if 'a' in self.modals:
                if self.graph_type == 'relation':
                    features_a, edge_index, edge_norm, edge_type, edge_index_lengths = batch_graphify(emotions_a, qmask, seq_lengths, self.window_past, self.window_future, self.edge_type_mapping, self.att_model, self.no_cuda)
                else:
            ...
            if 'v' in self.modals:
                if self.graph_type == 'relation':
                    features_v, edge_index, edge_norm, edge_type, edge_index_lengths = batch_graphify(emotions_v, qmask, seq_lengths, self.window_past, self.window_future, self.edge_type_mapping, self.att_model, self.no_cuda)
                else:
            ...
            if 'l' in self.modals:
                if self.graph_type == 'relation':
                    features_l, edge_index, edge_norm, edge_type, edge_index_lengths = batch_graphify(emotions_l, qmask, seq_lengths, self.window_past, self.window_future, self.edge_type_mapping, self.att_model, self.no_cuda)
                else:
        ...
        if self.graph_type=='relation':
            ...
            emotions_feat,modalbias,acosbias = self.graph_model(features_a, features_v, features_l, seq_lengths, qmask)
            testa = torch.sum(emotions_feat)

            qmask = torch.cat([qmask[:x, i, :] for i, x in enumerate(seq_lengths)], dim=0)
            spk_idx = torch.argmax(qmask, dim=-1)
            # spk_idx = spk_idx.cpu().numpy()
            # emotions_feat = emotions_feat.detach().cpu().numpy()

            row_number = sum(seq_lengths)
            #emotions_feat_temp_all = torch.zeros(row_number,900).cuda()
            start = 0
            bias_temp = torch.zeros(row_number).cuda()

            if self.dataset == 'MELD':

                for j in seq_lengths:
                    emotions_feat_temp = emotions_feat[start:start+j,:]
                    spk_idx_temp = spk_idx[start:start+j]
                    emotions_feat_temp4 = torch.zeros(j, 900).cuda()
                    emotions_feat_temp3 = torch.zeros(j, 900).cuda()
                    emotions_feat_temp2 = torch.zeros(j, 900).cuda()
                    emotions_feat_temp1 = torch.zeros(j,900).cuda()
                    emotions_feat_temp0 = torch.zeros(j,900).cuda()
                    for i in np.arange(len(spk_idx_temp)):
                        number_a = spk_idx_temp[i]
                        if number_a == 4:
                            emotions_feat_temp4[i] += emotions_feat_temp[i]
                        if number_a == 3:
                            emotions_feat_temp3[i] += emotions_feat_temp[i]
                        if number_a == 2:
                            emotions_feat_temp2[i] += emotions_feat_temp[i]
                        if number_a == 1:
                            emotions_feat_temp1[i] += emotions_feat_temp[i]

                            # emotions_feat_i = torch.abs(emotions_feat[i])
                            # means_np = emotions_feat_i.mean(axis=0)
                            # means_np = means_np.detach().cuda()
                            # means_np = torch.full((900,1),means_np)
                            # means_np = means_np.squeeze().cuda()

                        if number_a == 0:
                            emotions_feat_temp0[i] += emotions_feat_temp[i]
                            testea = torch.sum(emotions_feat_temp0[i])
                    emotions_feat_temp4_mean = torch.mean(emotions_feat_temp4, dim=-2)
                    emotions_feat_temp3_mean = torch.mean(emotions_feat_temp3, dim=-2)
                    emotions_feat_temp2_mean = torch.mean(emotions_feat_temp2, dim=-2)
                    emotions_feat_temp1_mean = torch.mean(emotions_feat_temp1,dim = -2)
                    emotions_feat_temp0_mean = torch.mean(emotions_feat_temp0,dim = -2)
                    bias_temp4 = torch.zeros(j).cuda()
                    bias_temp3 = torch.zeros(j).cuda()
                    bias_temp2 = torch.zeros(j).cuda()
                    bias_temp1 = torch.zeros(j).cuda()
                    bias_temp0 = torch.zeros(j).cuda()

                    for i in np.arange(len(spk_idx_temp)):
                        number_a = spk_idx_temp[i]
                        if number_a == 4:
                            a10 = torch.sum(torch.pow(emotions_feat_temp4_mean - emotions_feat_temp[i], 2))
                            a9 = torch.sqrt(a10)
                            bias_temp4[i] = a9
                        if number_a == 3:
                            a8 = torch.sum(torch.pow(emotions_feat_temp3_mean - emotions_feat_temp[i], 2))
                            a7 = torch.sqrt(a8)
                            bias_temp3[i] = a7
                        if number_a == 2:
                            a6 = torch.sum(torch.pow(emotions_feat_temp2_mean - emotions_feat_temp[i], 2))
                            a5 = torch.sqrt(a6)
                            bias_temp2[i] = a5
                        if number_a == 1:
                            a1 = torch.sum(torch.pow(emotions_feat_temp1_mean - emotions_feat_temp[i], 2))
                            a2 = torch.sqrt(a1)
                            #temp1 = torch.matmul(a1,emotions_feat_temp[i])
                            bias_temp1[i] = a2
                        if number_a == 0:
                            testa = torch.pow(emotions_feat_temp0_mean - emotions_feat_temp[i], 2)
                            a3 = torch.sum(testa)
                            a4 = torch.sqrt(a3)
                            #temp0 = torch.matmul(a3, emotions_feat_temp[i])
                            bias_temp0[i] = a4

                    bias_temp[start:start+j] += bias_temp4+bias_temp3+bias_temp2+bias_temp1+bias_temp0
                    start += j
                ...
                for j in seq_lengths:
                    ...
                    for i in np.arange(len(spk_idx_temp)):
                        ...
                        if number_a == 1:
                            ...
                            #余弦相似度
                            a1_sim = torch.sum(emotions_feat_temp1_mean.mul(emotions_feat_temp[i]))/ (torch.linalg.norm(emotions_feat_temp1_mean) * torch.linalg.norm(emotions_feat_temp[i]))

                            #dia_sim = 1 - torch.acos(a1_sim) / np.pi
                            dia_sim = torch.acos(a1_sim) / np.pi
                            #bias_temp0[i] = a1_sim
                            if dia_sim<0:
                                print('有负数')
                            bias_temp1[i] = dia_sim * 0.99999
                            #以上


                        if number_a == 0:
                            a3 = torch.sum(torch.pow(emotions_feat_temp0_mean - emotions_feat_temp[i], 2))
                            a4 = torch.sqrt(a3)
                            # temp0 = torch.matmul(a3, emotions_feat_temp[i])
                            bias_temp0[i] = a4

                            # 余弦相似度
                            a0_sim = torch.sum(emotions_feat_temp0_mean.mul(emotions_feat_temp[i]) )/ (
                                        torch.linalg.norm(emotions_feat_temp0_mean) * torch.linalg.norm(
                                    emotions_feat_temp[i]))

                            #dia_sim = 1 - torch.acos(a0_sim) / np.pi   #（）
                            dia_sim =  torch.acos(a0_sim) / np.pi

                            if dia_sim<0:
                                print('有负数')
                            #bias_temp0[i] = a0_sim
                            #bias_temp0[i] = dia_sim * 0.99999
                            # 以上

                    bias_temp[start:start + j] +=  bias_temp1 + bias_temp0
            #以上为说话人一致性模块
            ...
            #门控机制代码
            # merge_representation = torch.cat((fusion_embs, gaz_embeds_cat), dim=-1)
            # merge_representation = torch.cat((word_embs, gaz_embeds_cat), dim=-1)
            # gate_value = torch.sigmoid(self.gate(merge_representation))
            # gate_value1 = torch.sigmoid(self.gate1(merge_representation))
            # gated_converted_embed = torch.mul(gate_value, gaz_embeds_cat)
            # #gated1_converted_embed = torch.mul(gate_value1, fusion_embs)
            # gated1_converted_embed = torch.mul(gate_value1, word_embs)
            # word_input = torch.cat([gated1_converted_embed,gated_converted_embed],dim=-1)
            ...
            print("There are no such kind of graph")
        return log_prob, edge_index, edge_norm, edge_type, edge_index_lengths,bias_temp,modalbias,acosbias
