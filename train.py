...
from model import MaskedNLLLoss, LSTMModel, GRUModel, DialogRNNModel, DialogueGCNModel, MaskedMSELoss, FocalLoss, GRUnet
...
def get_IEMOCAP_loaders(batch_size=32, valid=0.1, num_workers=0, pin_memory=False):
    ...
    train_sampler, valid_sampler = get_train_valid_sampler(trainset, valid, 'IEMOCAP')
...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, modals, optimizer=None, train=False,
    ...
    for data in dataloader:
        ...
        # umask1 = data[4]
        # dia_len = [(umask1[j] == 1).nonzero().tolist()[-1][0] + 1 for j in range(len(umask1))]
        ...
        # non_len= []
        # for i in dia_len:
        #     non_len.append(int(i * 0.1))
        # ### 设置每段对话的前百分比话语特征为0
        # j = 0
        # start = 0
        # for i in dia_len:
        #
        #     g = textf[start:start+non_len[j], :]
        #     textf[start:start+non_len[j], :] = 0 * g
        ...
        if args.multi_modal and args.mm_fusion_mthd == 'gated':
            ...
            log_prob, e_i, e_n, e_t, e_l, bias, modalbias,acosbias = model(textf, qmask, umask, lengths, acouf, visuf)
        else:
        ...
        if args.Dataset == 'MELD':
            ...
            # loss = 2.5*loss
            # #bias = 0.00005 * bias
            # modalbias = 0.001 * modalbias
            # loss = loss - modalbias

            loss = 0.499 * loss  # 0.499
            bias = 0.003 * bias  # 0.003
            modalbias = 0.0003 * modalbias  # 0.0003
            acosbias = 0.006 * acosbias  # 0.006
            loss = loss + bias - modalbias + acosbias

        if args.Dataset == 'IEMOCAP':
            loss = loss_function(log_prob, label)

            loss = 1*loss#0.499
            bias = 0.1 * bias#0.003   0.01+0.01:6917
            modalbias = 0.1* modalbias#0.0003

            loss = loss #+bias + modalbias #+acosbias        #此项为IEMOCAP的loss
            #loss = 1.001*loss
            # a1 = len(label)
            # def attention_linear( converted_loss, converted_bias_embed, bias_embed):
            #
            #
            #     # keys_size = converted_bias_embed.size(1)
            #     # converted_loss = converted_loss.unsqueeze(1).expand(-1, keys_size, -1)  # batch_size, keys_number,hidden_dim
            #     attention_inputs = torch.tanh(converted_loss + converted_bias_embed)
            #     # att_weights = self.madality_attetion(attention_inputs).squeeze(2)  # batch_size, keys_number
            #     att_weights = F.softmax(attention_inputs, dim=-1)  # batch_size, 1, keys_number
            #
            #     att_vector = torch.mul(att_weights, bias_embed)  # batch_size, 2048
            #     return att_vector, att_weights
            #
            # # apply entity-based attention mechanism to obtain different image representations
            # bias_embed = bias
            # converted_bias_embed = bias_embed
            # converted_loss = loss
            # #vis2text = nn.Linear()
            # emdeding = nn.Linear(2*a1,a1).cuda()
            #
            # # att_vector: batch_size, 2048
            # att_vector, att_weights = attention_linear(converted_loss, converted_bias_embed, bias_embed)
            # #converted_att_vis_embed = self.vis2text(torch.tanh(att_vector))  # att_vector: batch_size, hidden_dim
            #
            # merge_representation = torch.cat((converted_loss, att_vector), dim=-1)
            # merge_representation = emdeding(merge_representation)
            # gate_value = torch.sigmoid( merge_representation)  # batch_size, hidden_dim
            # gated_converted_att_vis_embed = torch.mul(gate_value, converted_bias_embed)
            # #loss =15*loss + gated_converted_att_vis_embed


        # row_number = sum(lengths)
        # grunet = nn.GRU(900,row_number,num_layers = 1,dropout = 0.23).cuda()
        # loss1 = loss.unsqueeze(1)
        # loss2 = loss1.unsqueeze(1)
        # trans = nn.Linear(1,900).cuda()
        # loss2 = trans(loss2)
        # loss2,a = grunet(loss2)
        # loss2 = loss2.squeeze(1)
        # loss2 = torch.sum(loss2,dim=1)



        # awl = AutomaticWeightedLoss(3)  # we have 3 losses
        # loss1 = (loss)
        # loss2 = (bias)
        # loss3 = (modalbias)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.000222, metavar='LR', help='learning rate')#0.00022:6814,0.000221:68.9,

    parser.add_argument('--l2', type=float, default=0.0000299, metavar='L2', help='L2 regularization weight')#18.22.299,0.000021:68.65  0.0000299

    parser.add_argument('--rec-dropout', type=float, default=0.1, metavar='rec_dropout', help='rec_dropout rate')

    parser.add_argument('--dropout', type=float, default=0.31, metavar='dropout', help='dropout rate')

    parser.add_argument('--batch-size', type=int, default=58, metavar='BS', help='batch size')#48：6878  56：6898  58:6907
#第二个点：8：65.41；
    parser.add_argument('--epochs', type=int, default=500, metavar='E', help='number of epochs')
    ...
    parser.add_argument('--graph_type', default='MMGCN2', help='relation/GCN3/DeepGCN/MMGCN/MMGCN2')
    ...
    parser.add_argument('--graph_construct', default='direct',
                        help='single/window/fc for MMGCN2; direct/full for others')
    ...
    if args.Dataset == 'MELD':
        ...
        if args.class_weight:
            if args.graph_model:
                loss_function = nn.NLLLoss(loss_weights.cuda() if cuda else loss_weights, reduction='none')
            else:
    ...
    for e in range(n_epochs):
        ...
        if (e + 1) % 10 == 0:
            ...
            print('目前F-Score：', max(all_fscore))
            print(best_label)
    ...
    print('最终F-Score:', max(all_fscore))
